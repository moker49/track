...
def refresh_retry_after(db: sqlite3.Connection, show_id: int) -> str | None:
    row = db.execute(
        "SELECT retry_after FROM show_metadata_refresh_failures WHERE show_id = ?",
        (show_id,),
    ).fetchone()
...
def record_refresh_failure(
    ...
    *,
    ...
    row = db.execute(
        "SELECT failure_count FROM show_metadata_refresh_failures WHERE show_id = ?",
        (show_id,),
    ).fetchone()
    ...
    db.execute(
        """
        INSERT INTO show_metadata_refresh_failures (
            show_id, failure_count, last_attempt_at, retry_after, error
        ) VALUES (?, ?, ?, ?, ?)
        ...
        """,
        (show_id, failure_count, attempted_at, retry_after, error),
    )
    db.commit()
    return retry_after


def clear_refresh_failure(db: sqlite3.Connection, show_id: int) -> None:
    db.execute("DELETE FROM show_metadata_refresh_failures WHERE show_id = ?", (show_id,))
    db.commit()
