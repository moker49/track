...
class TrackAppTest(unittest.TestCase):
    ...
    def test_library_filter_and_sort_dialog_is_in_the_shell(self):
        ...
        self.assertNotIn(b'data-filter-state=', home.data)
        self.assertEqual(home.data.count(b'class="tv-list-heading-toggle"'), 2)
        ...
        self.assertIn('preferences.state = preferences.state === "ACTIVE" ? "ARCHIVED" : "ACTIVE";', javascript)
        self.assertIn("button.disabled = searching;", javascript)
        self.assertIn("if (icon) icon.hidden = searching;", javascript)
        self.assertEqual(home.data.count(b">swap_vert</span>"), 2)
        ...
        self.assertIn(".filter-button {\n  color: var(--on-surface-muted);", css)
