...
class TrackAppTest(unittest.TestCase):
    ...
    def test_single_page_shell_contains_primary_views(self):
        ...
        self.assertIn(b'class="material-symbols-rounded is-filled" aria-hidden="true">account_circle</span>', home.data)
        ...
        self.assertIn(b'data-view="profile"', home.data)
        self.assertIn(b'data-profile-back', home.data)
        self.assertIn(b'data-profile-tab="diary"', home.data)
        self.assertIn(b'data-profile-tab="statistics"', home.data)
        self.assertIn(b'data-profile-panel="diary"', home.data)
        self.assertIn(b'data-profile-panel="statistics"', home.data)
        ...
        self.assertIn(".profile-top-app-bar {", css)
        self.assertIn(".profile-tabs {", css)
        self.assertIn('.profile-tabs button[aria-selected="true"]::after {', css)
        self.assertIn(".app-content.is-profile-view {", css)
        self.assertIn("min-height: calc(80px + env(safe-area-inset-top));", css)
        self.assertIn("background: var(--surface-card);", css)
        self.assertIn(".profile-floating-chrome {", css)
        self.assertIn("@keyframes profile-page-enter-right", css)
        self.assertIn("html.profile-transition-enter::view-transition-new(root)", css)
        ...
        self.assertIn("function selectProfileTab(tabName", javascript)
        self.assertIn("function setProfileFloatingChromeVisible(visible, { instant = false } = {})", javascript)
        self.assertIn("function prepareProfileFloatingChrome()", javascript)
        self.assertIn("function transitionProfileView(change, direction)", javascript)
        self.assertIn("function openProfileFromTrigger(_trigger)", javascript)
        self.assertIn('transitionProfileView(() => showView("profile", "push"), "enter")', javascript)
        self.assertIn('showView("profile", "push")', javascript)
        self.assertIn('window.history.state.view === "profile"', javascript)
        self.assertIn('bottomChrome.hidden = profileOpen', javascript)
    ...
    def test_tv_controls_share_a_bottom_inline_bar(self):
        ...
        self.assertEqual(home.data.count(b"data-tv-dropdown-toggle="), 1)
        self.assertIn(b'data-tv-dropdown-toggle="combined"', home.data)
        self.assertIn(b'data-tv-dropdown-menu="combined"', home.data)
        self.assertIn(b"data-tv-progress-label>Filter</span>", home.data)
        self.assertIn(b"data-tv-sort-label>Sort</span>", home.data)
        self.assertIn(b"data-tv-combined-divider", home.data)
