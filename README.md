...
The browser loads one persistent application shell at `/`. Schedule is the first and default destination, followed by Watching, Archive, and Discover. These views switch in place without changing the URL. Selecting a show renders its compact overview first, then loads seasons and episodes in the background. Both fragments are cached in memory for seamless repeat visits during the current browser session. Stored TMDB metadata is refreshed in the background only when its last refresh is at least 24 hours old, or immediately when Refresh is chosen from the show-detail menu. Existing details stay visible until the refreshed overview and episode list are ready. The outer app shell and navigation are never replaced.

Schedule opens on the Backlog tab, which displays only the oldest released episode that has not been watched or skipped for each Watching show. Watch records a normal watch event; Skip advances the queue without changing watch history and offers a temporary Undo action in the snackbar. The Upcoming tab presents future-dated episodes for Watching shows as a month-grouped timeline with release-day markers and live day countdowns. Specials and Archived shows are excluded from both tabs.
